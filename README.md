...
- **Sidebar quick-toggle buttons** — an identical two-row button panel at the bottom of both sidebars: `SB` switches between the Annotations and Comments sidebars, and the rest fire the visibility/formatting commands directly (`A-F`/`B-V`/`B-F` for annotations, `C-V`/`C-F`/`B-V`/`B-F` for comments) — hover any button to see the full command name
